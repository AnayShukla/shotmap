...
with tab1:
    ...
    if button and input1 and season:
        if not season.isdigit() or len(season) != 4:
            ...
            with st.spinner(
                "Generating shot map, this will take a few minutes...", show_time=True
            ):
                try:

                    def load_player_mappings():
                        with open("player_mappings.json", "r", encoding="utf-8") as f:
                            return json.load(f)
                    ...
                    model = SentenceTransformer("gemini-embedding-exp-03-07")
                    ...
                    league_player_data = client.league(league=league).get_player_data(
                        season=season
                    )
